...
shared ({ caller = _owner }) actor class Token(
    ...
    public shared ({ caller }) func mint(args : ICRC1.Mint) : async ICRC1.TransferResult {
        let result = await* ICRC1.mint(token, args, caller);

         let mintedPrice : Types.Price = {
            currency = #kt; 
            amount = args.amount; 
        };

        let callerText = Principal.toText(caller);
        let userCanister = actor(callerText) : actor {
            addToWallet : shared(price: Types.Price) -> async();
        };
        
        await userCanister.addToWallet(mintedPrice);
        return result;
