...
shared(_init_msg) actor class NFTCanister(_args : {
    ...
    public shared(msg) func icrc7_transfer(args: Types.TransferArgs) : async [?ICRC7.TransferResult] {
        return icrc7().transfer<system>(msg.caller, [args]);
    };
    ...
    public shared(msg) func test_create_collection<system>() : async [ICRC7.SetNFTResult] {
        let nftData = [
            {
                name = "Image 1";
        ...
        let requestNew : [ICRC7.SetNFTItemRequest] = [{
                token_id = 109238102;
                owner = ?{owner = icrc7().get_state().owner; subaccount = null};
                metadata = #Class([
                    {
                        name = "icrc7:metadata:uri:image";
                        value = #Text("naasiodjmnaosidj");
                        immutable = true;
                    },
                ]);
                memo = null;
        ...
        D.print("RequestNew : " # debug_show(requestNew));

        let result = icrc7().set_nfts<system>(icrc7().get_state().owner, requestNew, true);
    ...
    public func returnPrincipal() : async Principal{
