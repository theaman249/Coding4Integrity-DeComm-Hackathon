...
<header class="w-full mx-auto py-3 lg:py-7 z-20 transition-all duration-300 ease-in-out border-b-4 border-zinc-400 top-0 absolute bg-white">
    <nav class="grid grid-cols-12 gap-4 items-center py-2 text-stone-500 hover:text-stone-500 focus:text-stone-700 lg:py-1 data-te-navbar-ref">
        <div class="col-span-4 lg:col-span-3 lg:flex lg:justify-start">
            <div class="ml-5 lg:ml-10">
                <a href="/" class="font-bold text-xl lg:text-2xl text-black hover:cursor-pointer active:text-black/50" on:click|preventDefault={goHomeTitle}>D&nbspE&nbspC&nbspO&nbspM&nbspM</a>
            </div>
        </div>
        <div class="col-span-8 lg:col-span-6 flex pl-5 lg:pl-0 lg:justify-center min-w-full justify-end">
        ...
        <div class="hidden lg:col-span-3 lg:flex justify-end lg:mr-10 gap-x-5 inset-0">
            ...
            {/key}
            <div>
            <button class="hover:bg-zinc-200 hover:rounded-lg flex items-center justify-center cursor-pointer" on:click={() => goto("/cart")}>
                <IconShoppingCart size={48} stroke={1} color={"#71717a"} />
                <div class="absolute top-[2.54rem] right-[3.25rem] flex items-center justify-center w-5 h-5 text-zinc-500 text-xs font-bold rounded-full">{$cart.value}</div>
            </button>
        </div>
    </nav>
</header>
...
{#if $accountType.value === "Personal Account" && $page.url.pathname === "/" && innerWidth < 1024}
    <div class="fixed bottom-0 bg-white z-20 w-full border-t-2 border-zinc-300 p-2">
