...
<button use:melt={$trigger} class="flex items-center justify-center gap-2 px-4 py-2 border-2 rounded-[20px] font-semibold w-full md:w-[50vh] xl:w-full">
	<SearchIcon />
	<span class="hidden sm:block">Search</span>
	<div class="hidden sm:block">
		<kbd class="px-2 text-search-kbd-txt bg-search-kbd-bg border border-search-kbd-border rounded">{platform === 'MacIntel' ? '⌘' : 'Ctrl'}</kbd> + <kbd class="px-2 py-1 text-search-kbd-txt bg-search-kbd-bg border border-search-kbd-border rounded">K</kbd>
	</div>
</button>
        ...
        <div class="min-h-[85vh] max-h-[85vh] xl:min-h-[80vh] xl:max-h-[80vh] p-2 bg-white overflow-y-auto scrollbar-thin rounded-b-xl border-zinc-400 border-x-[3px] border-b-[3px]" id="searchingBox">
          ...
          {#if results && results.length > 0}
              <div class="grid grid-cols-12 2xl:mt-5 xl:mt-0">
                <div class="col-span-12 grid grid-cols-12 justify-center gap-10 p-2 overflow-x-hidden">
